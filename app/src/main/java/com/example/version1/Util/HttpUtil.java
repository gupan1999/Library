            ...
            public void run() {
                try {
                    ...
                    for (int i=0;i<requestjsons.size();i++) {
                        ...
                        Request request = new Request.Builder().url("http://10.128.245.151:9999" + requestjsons.get(i)).build();  //连缀设置url地址的Request对象
                        Call call = client.newCall(request);
                        call.enqueue(new Callback() {
                            @Override
                            public void onFailure(Call call, IOException e) {
            ...
            public void run() {
                try {
                    OkHttpClient client = new OkHttpClient.Builder().connectTimeout(5, TimeUnit.SECONDS) //连接超时
                        ...
                        Request request = new Request.Builder().url("http://10.128.245.151:9999" + requestjsons.get(0)).build();  //连缀设置url地址的Request对象
                        Call call = client.newCall(request);
                        call.enqueue(new Callback() {
                            @Override
                            public void onFailure(Call call, IOException e) {
